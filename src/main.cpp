...
uint256 hashGenesisBlock("0x96a415f0b0cf527c5f3168d861354e069a29a7dc496355715ec21d15f84ea001");
...
const string strMessageMagic = "BitMiles Signed Message:\n";
...
else if(nHeight >= 121 && nHeight <= 44640) {
	nSubsidy = 500 * COIN;
    }

    // Hereafter, subsidy is cut in half every 1051200 blocks, which will occur every 2 years and end at year 8
...
static const int64 nTargetTimespan = 1.0 *24 * 60 * 60;	// BitMiles: 1 day
static const int64 nTargetSpacing = 60; // BitMiles: 60 Seconds
...
unsigned int static GetNextWorkRequired_Original(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // BitMiles: This fixes an issue where a 51% attack can change difficulty at will.
...
static unsigned int GetNextWorkRequired_DigiShieldV2(const CBlockIndex* pindexLast, const CBlock *pblock, int ampfilter)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal BitMiles miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitMiles", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x330d6865321f8d8d37ca48c077b86eeb6ea6ef9d2f3d2c92c5ce6b10606d6370");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "20141201BTM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1417542480; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1239544;

        if (fTestNet)
        {
            block.nTime    = 1417453540;
            block.nNonce   = 225292;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb2ffa5dfa76d7a6f0bba7b6442b4043d7e6cbbae97fe0fa40da67d51883b9b69"));
